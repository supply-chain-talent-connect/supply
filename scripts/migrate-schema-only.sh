...
# Migrates only schema (not data) from NAS PostgreSQL to Cloud SQL
...
BACKUP_FILE="$BACKUP_DIR/directus-schema-$TIMESTAMP.sql"
CLEANED_FILE="$BACKUP_DIR/directus-schema-$TIMESTAMP-cleaned.sql"
LOG_FILE="$BACKUP_DIR/directus-schema-$TIMESTAMP.log"
...
# Main schema migration process
main() {
    log_info "Starting schema-only migration from NAS to Cloud SQL..."
    ...
    # Step 1: Dump schema only from NAS
    log_info "Dumping schema only from NAS PostgreSQL..."
    local start_time=$(date +%s)
    
    if ! ssh root@$NAS_HOST "docker exec $NAS_CONTAINER pg_dump -U $NAS_USER -d $NAS_DB --schema-only --no-owner --no-acl --clean --if-exists" > "$BACKUP_FILE" 2>>"$LOG_FILE"; then
        log_error "Failed to dump schema from NAS - check log for details"
        exit 1
    fi
    
    # Check if backup was successful
    if [ ! -s "$BACKUP_FILE" ]; then
        log_error "Schema backup file is empty or doesn't exist"
        exit 1
    fi
    ...
    log_info "Schema dump created successfully: $FILE_SIZE (took ${duration}s)"
    
    # Step 2: Clean backup for Cloud SQL compatibility
    log_info "Cleaning schema backup for Cloud SQL compatibility..."
    start_time=$(date +%s)
    ...
    log_info "Schema backup cleaning completed (took ${duration}s)"
    
    # Step 3: Import schema to Cloud SQL
    log_info "Importing schema to Cloud SQL..."
    start_time=$(date +%s)
    
    export PGPASSWORD="$CLOUD_SQL_PASSWORD"
    if ! psql -h "$CLOUD_SQL_IP" -U "$CLOUD_SQL_USER" -d "$CLOUD_SQL_DB" -f "$CLEANED_FILE" >>"$LOG_FILE" 2>&1; then
        log_error "Failed to import schema to Cloud SQL - check log for details"
        exit 1
    fi
    
    end_time=$(date +%s)
    duration=$((end_time - start_time))
    log_info "Schema import completed successfully (took ${duration}s)"
    
    log_info "Schema-only migration completed successfully!"
    log_info "Summary:"
    log_info "- Source: $NAS_HOST ($NAS_DB)"
    log_info "- Destination: $CLOUD_SQL_IP ($CLOUD_SQL_DB)"
    log_info "- Schema file: $FILE_SIZE"
