...
# Migrates only data (not schema) from NAS PostgreSQL to Cloud SQL
...
BACKUP_FILE="$BACKUP_DIR/directus-data-$TIMESTAMP.sql"
CLEANED_FILE="$BACKUP_DIR/directus-data-$TIMESTAMP-cleaned.sql"
LOG_FILE="$BACKUP_DIR/directus-data-$TIMESTAMP.log"
...
main() {
    log_info "Starting data-only migration from NAS to Cloud SQL..."
    ...
    # Step 1: Dump data only from NAS
    log_info "Dumping data only from NAS PostgreSQL..."
    local start_time=$(date +%s)
    
    if ! ssh root@$NAS_HOST "docker exec $NAS_CONTAINER pg_dump -U $NAS_USER -d $NAS_DB --data-only --disable-triggers --no-owner --no-acl" > "$BACKUP_FILE" 2>>"$LOG_FILE"; then
        log_error "Failed to dump data from NAS - check log for details"
        exit 1
    fi
    
    # Check if backup was successful
    if [ ! -s "$BACKUP_FILE" ]; then
        log_error "Data backup file is empty or doesn't exist"
        exit 1
    fi
    ...
    log_info "Data dump created successfully: $FILE_SIZE (took ${duration}s)"
    
    # Step 2: Clean backup for Cloud SQL compatibility
    log_info "Cleaning data backup for Cloud SQL compatibility..."
    start_time=$(date +%s)
    ...
    log_info "Data backup cleaning completed (took ${duration}s)"
    ...
    if ! psql -h "$CLOUD_SQL_IP" -U "$CLOUD_SQL_USER" -d "$CLOUD_SQL_DB" -f "$CLEANED_FILE" >>"$LOG_FILE" 2>&1; then
        log_error "Failed to import data to Cloud SQL - check log for details"
        exit 1
    fi
    
    end_time=$(date +%s)
    duration=$((end_time - start_time))
    log_info "Data import completed successfully (took ${duration}s)"
    
    log_info "Data-only migration completed successfully!"
    log_info "Summary:"
    log_info "- Source: $NAS_HOST ($NAS_DB)"
    log_info "- Destination: $CLOUD_SQL_IP ($CLOUD_SQL_DB)"
    log_info "- Data file: $FILE_SIZE"
